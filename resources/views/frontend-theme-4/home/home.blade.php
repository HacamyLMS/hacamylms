...
@endphp
<section class="hero-banner" data-backg round="{{ getImageFile($bannerImage) }}" sty
  le="background-size: contain; background-position: center bottom;">
  <div class="container">
  ...
  <div class="container">
    <div class="product-varity-content">
      ...
      <div class="row rg-24">
        ...
        <div class="col-lg-4 col-md-6">
          <div class="course-item-two">
            <div class="img">
              <img src="{{getImageFile($product->thumbnail_path)}}" alt="{{$product->title}}" />
            </div>
            <div class="content">
              ...
              <button class="btn-fill-lan addToCart" data-product_id="{{ $product->id }}" data-quantity=1
                data-route="{{ route('student.addToCart') }}">{{ __('Add To Cart') }}
                <i class="fa fa-arrow-right"></i></button>
              @else
              <button class="btn-fill-lan">{{ __('Out of stock') }} <i class="fa fa-arrow-right"></i></button>
              @endif
            </div>
          </div>
        </div>
      ...
      <div class="pt-sm-41 pt-20 d-flex justify-content-center">
        <a href="{{ route('lms_product.frontend.list') }}" class="btn-outline-lan">{{__('View All Product ')}}<i
            class="fa fa-arrow-right"></i></a>
      </div>
    </div>
  </div>
        ...
        <div class="row justify-content-between align-items-center rg-20">
          <div class="col-lg-8">
            <div class="d-flex align-items-lg-center align-items-start g-26">
              ...
              <div class="content">
                <h4 class="title">{{ __(get_option('upcoming_course_title')) }}</h4>
                <p class="text">{{ __(get_option('upcoming_course_title')) }}</p>
              </div>
            </div>
          </div>
          <div class="col-lg-2">
    ...
    <div class="top-rated-course-content">
      ...
      <div class="title-wrap">
        <div class="row justify-content-between align-items-end rg-20">
          <div class="col-lg-8">
            <div class="d-flex align-items-start g-26">
              <div class="icon d-flex max-w-70 flex-shrink-0">
                <img src="{{ getImageFile(get_option('top_instructor_logo')) }}" alt="" />
              </div>
              <div class="content">
                <h4 class="title">{{ __(get_option('top_instructor_title')) }}</h4>
                <p class="text">{{ __(get_option('top_instructor_subtitle')) }}</p>
              </div>
            </div>
          </div>
          <div class="col-lg-2">
            <div class="d-flex justify-content-lg-end">
              <a href="{{ route('instructor') }}" class="btn-outline-lan">{{__('View All Instructor')}} <i
                  class="fa fa-arrow-right"></i>
              </a>
            </div>
          </div>
      ...
      <div class="row rg-20">
        ...
        <div class="col-xl-3 col-lg-4 col-sm-6">
          <a href="{{ route('userProfile', $instructorUser->id) }}">
            <div class="instructor-item-one" data-background="{{ getImageFile(@$instructorUser->image_path) }}">
              <div class="content">
                ...
                <div class="d-flex justify-content-between align-items-center g-10 flex-wrap">
                  ...
                  <div class="d-flex align-items-center g-5">
                    <div class="icon d-flex">
                      <img src="{{asset('frontend-theme-4/assets/images/star-full.svg')}}" alt="" />
                    </div>
                    <p class="fs-14 fw-500 lh-15 text-main-color">{{ number_format(@$average_rating, 1) }}</p>
                  </div>
                  <!--  -->
                  <p class="fs-14 fw-500 lh-15 text-main-color">{{@$instructorUser->enrollment_students_count}}
                    {{__('students')}}</p>
                  <!--  -->
                  <p class="fs-14 fw-500 lh-15 text-main-color">{{@$instructorUser->courses->count()}} {{__('courses')}}
                  </p>
                </div>
              </div>
            </div>
  ...
  <div class="container">
    <div class="price-plan-content">
      ...
      <div class="price-plan-items">
        ...
        @php
        $isCurrentMonthly = (($subscription->id == @$mySubscriptionPackage->package_id &&
        @$mySubscriptionPackage->subscription_type) == SUBSCRIPTION_TYPE_MONTHLY);
        $isCurrentYearly = (($subscription->id == @$mySubscriptionPackage->package_id &&
        @$mySubscriptionPackage->subscription_type) == SUBSCRIPTION_TYPE_YEARLY);
            ...
            <div class="col-md-3">
              <div class="price-plan-iconPrice">
                ...
                <div class="d-flex align-items-center g-10">
                  @if($priceMonthly < 1) <h4 class="planPrice planPrice-monthly-amount">{{__('Full Free')}}</h4>
                    @else
                    <h4 class="planPrice planPrice-monthly-amount">
                      {{ $priceMonthly.($currencySymbol ?? get_currency_symbol()) }}</h4>
                    @if($priceMonthly != $oldPriceMonthly)
                    <h4
                      class="planPrice planPrice-monthly-amount text-decoration-line-through fs-14 lh-14 text-para-text-alt">
                      {{ $oldPriceMonthly.($currencySymbol ?? get_currency_symbol()) }}</h4>
                    @endif
                    @endif
                    @if($priceYearly < 1) <h4 class="planPrice planPrice-yearly-amount">{{__('Full Free')}}</h4>
                      @else
                      <h4 class="planPrice planPrice-yearly-amount">
                        {{ $priceYearly.($currencySymbol ?? get_currency_symbol()) }}</h4>
                      @if($priceYearly != $oldPriceYearly)
                      <h4
                        class="planPrice planPrice-yearly-amount text-decoration-line-through fs-14 lh-14 text-para-text-alt">
                        {{ $oldPriceYearly.($currencySymbol ?? get_currency_symbol()) }}</h4>
                      @endif
                      @endif
                </div>
              </div>
            ...
            <div class="col-md-6">
              <div class="price-plan-listDivider">
                <div class="plan-divider">
                  <img src="{{asset('frontend-theme-4/assets/images/price-plan-divider-lan.svg')}}" alt="" />
                </div>
              ...
              <div class="price-plan-btn">
                <form method="post" action="{{ route('student.subscription.checkout', $subscription->uuid) }}">
                  ...
                  <button type="submit" {{ ($disabledMonthly) ? 'disabled' : '' }}
                    class="{{ ($disabledMonthly) ? 'disabled-btn' : '' }} linkBtn monthly-btn">{{ ($disabledMonthly && $isCurrentMonthly) ? __("Current Plan") : __("Get Started") }}</button>
                  <button type="submit" {{ ($disabledYearly) ? 'disabled' : '' }}
                    class="{{ ($disabledYearly) ? 'disabled-btn' : '' }} linkBtn yearly-btn d-none">{{ ($disabledYearly && $isCurrentYearly) ? __("Current Plan") : __("Get Started") }}</button>
                </form>
              </div>
            </div>
          </div>
    ...
    <div class="testimonial-section-content">
      ...
      <div class="">
        ...
        <div class="lan-testimonial-slider owl-carousel">
          ...
          {{--                                        </div>--}}
          {{--                                        <div class="info">--}}
          {{--                                            <h4 class="name">{{ __(get_option('customer_say_'.$customerSayItem.'_name')) }}
          </h4>--}}
          {{--                                            <p class="degi">{{ __(get_option('customer_say_'.$customerSayItem.'_position')) }}
          </p>--}}
          {{--                                        </div>--}}
          {{--                                    </div>--}}
          ...
          {{--                                        </div>--}}
          {{--                                        <div class="text-content">--}}
          {{--                                            <h4 class="title">{{ __(get_option('customer_say_'.$customerSayItem.'_comment_title')) }}
          </h4>--}}
          {{--                                            <p class="text">{{ __(get_option('customer_say_'.$customerSayItem.'_comment_description')) }}
          </p>--}}
          {{--                                        </div>--}}
  ...
  <div class="container">
    <div class="price-plan-content">
      ...
      <div class="d-flex justify-content-center">
        <ul class="nav nav-pills zTab-reset zTab-instructorOrganization" id="priceInsOrgpills-tab" role="tablist">
          <li class="nav-item" role="presentation">
            <button class="nav-link active" id="pills-instructor-tab" data-bs-toggle="pill"
      ...
      <div class="tab-content" id="priceInsOrgpills-tabContent">
        <div class="tab-pane fade show active" id="pills-instructor" role="tabpanel"
          ...
          <div class="price-plan-items">
            ...
            $disabledYearly = (!is_null($matched) && $mySaasPackage->subscription_type == SUBSCRIPTION_TYPE_YEARLY ?
            true : false);
            $disabledMonthly = ($disabledYearly) ? true : (!is_null($matched) && $mySaasPackage->subscription_type ==
            SUBSCRIPTION_TYPE_MONTHLY ? true : false);
            @endphp
            @foreach($instructorSaas as $index => $saas)
            @php
            $isCurrentMonthly = (($saas->id == @$mySaasPackage->package_id && @$mySaasPackage->subscription_type) ==
            SUBSCRIPTION_TYPE_MONTHLY);
            $isCurrentYearly = (($saas->id == @$mySaasPackage->package_id && @$mySaasPackage->subscription_type) ==
            SUBSCRIPTION_TYPE_YEARLY);
            ...
            <div class="price-plan-item">
              <div class="row align-items-center">
                <div class="col-md-3">
                  <div class="price-plan-iconPrice">
                    <div class="iconTitle">
                      <div class="icon"><img src="{{asset('frontend-theme-4/assets/images/plan-icon-1.svg')}}" alt="" />
                      </div>
                ...
                <div class="col-md-6">
                  <div class="price-plan-listDivider">
                    <div class="plan-divider"><img
                        src="{{asset('frontend-theme-4/assets/images/price-plan-divider-lan.svg')}}" alt="" />
                    </div>
                    <ul class="price-plan-list">
                      <li class="d-flex align-items-center g-10">
                        <div class="icon flex-shrink-0"><i class="fa fa-check"></i></div>
                      ...
                      <li class="d-flex align-items-center g-10">
                        <div class="icon flex-shrink-0"><i class="fa fa-check"></i></div>
                        <p class="text">
                          {{ __("Enable") .' '. $saas->subscription_course.' '.__('Subscription Course') }}</p>
                      </li>
                      <li class="d-flex align-items-center g-10">
                        <div class="icon flex-shrink-0"><i class="fa fa-check"></i></div>
                      ...
                      <li class="d-flex align-items-center g-10">
                        <div class="icon flex-shrink-0"><i class="fa fa-check"></i></div>
                        <p class="text">{{ __("Minimum of") .' '. $saas->admin_commission .'% '.__('sale commission')}}
                        </p>
                      </li>
                    </ul>
                  </div>
